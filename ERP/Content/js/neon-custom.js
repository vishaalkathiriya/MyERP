...
(function ($, window, undefined) {
    ...
    $(document).ready(function () {
        ...
        public_vars.$sidebarMenu.find(".sidebar-collapse-icon").on('click', function (ev) {
            ev.preventDefault(); var with_animation = $(this).hasClass('with-animation'); toggle_sidebar_menu(with_animation);
        });
        public_vars.$sidebarMenu.find(".sidebar-mobile-menu a").on('click', function (ev) {
            ev.preventDefault(); var with_animation = $(this).hasClass('with-animation');
            if (with_animation) {
        ...
        if ($.isFunction($.fn.niceScroll)) {
            ...
            if (fixed_sidebar.length == 1) {
                console.log("fixed");
                var fs_tm = 0;
                fixed_sidebar.niceScroll({ cursorcolor: '#454a54', cursorborder: '1px solid #454a54', railpadding: { right: 3 }, railalign: 'right', cursorborderradius: 1 });
        ...
        $('body').on('click', '.panel > .panel-heading > .panel-options > a[data-rel="reload"]', function (ev) {
            ...
            blockUI($this); $this.addClass('reloading');
            setTimeout(function () {
                unblockUI($this)
                ...
                draw: function () {
                    if (this.$.data('skin') == 'tron') {
                        ...
                        if (this.o.displayPrevious) {
                            ...
                            this.g.arc(this.xy, this.xy, this.radius - this.lineWidth, sa, ea, false); this.g.stroke();
                        }
                        this.g.beginPath();
        ...
        if ($.isFunction($.fn.slider)) {
            $(".slider").each(function (i, el) {
                var $this = $(el),
                    ...
                    $fill = $(fill), step = attrDefault($this, 'step', 1),
        ...
        if ($.isFunction($.fn.select2)) {
            $(".select2").each(function (i, el)
            {
                ...
                $this.select2(opts); $this.addClass('visible');
            });
            if ($.isFunction($.fn.niceScroll))
            {
        ...
        if ($.isFunction($.fn.selectBoxIt)) {
            $("select.selectboxit").each(function (i, el)
            {
                ...
                $this.addClass('visible'); $this.selectBoxIt(opts);
            });
        }
...
function setScrollTableHeight() {
    setTimeout(function () {
        var $tableScrollView = $('.table-scroll-view');
        var tableScrollViewOffset = $tableScrollView[0].getBoundingClientRect();
        var tableScrollViewOffsetTop = tableScrollViewOffset.top;
        var windowHeight = $(window).height();
        var footerHeight = $('#footer').height();
        var newHeight = windowHeight - (tableScrollViewOffsetTop + footerHeight + 10);
        $tableScrollView.height(newHeight);
    }, 100);
}
    ...
    if (public_vars.$sidebarMenu.length && public_vars.$sidebarMenu.hasClass('fixed') == false) {
        if (isxs()) {
            ...
            if (typeof reset_mail_container_height != 'undefined')
                reset_mail_container_height(); return;
        ...
        if (win_height > doc_height) { doc_height = win_height; }
...
function setup_sidebar_menu() {
    var $items_with_submenu = public_vars.$sidebarMenu.find('li:has(ul)'),
        submenu_options = { submenu_open_delay: 0.5, submenu_open_easing: Sine.easeInOut, submenu_opened_class: 'opened' },
    ...
    for (var i = 0; i < arr.length; i++) {
        if (arr[i] != "") {
            ...
            $("a[href='" + str + "']").parent().parent().parent().addClass("opened active root-level has-sub");
            $("a[href='" + str + "']").parent("li").addClass("active");
            $("a[href='" + str + "']").parent("li").parent('ul').addClass('visible');
        }
    }
    $("a[href='" + window.location.pathname + "']").parent().parent().parent().addClass("opened active root-level has-sub");
...
function menu_do_expand($submenu, $this, options) {
    ...
    var props_from = { opacity: .2, height: 0, top: -20 },
...
jQuery(public_vars, { hover_index: 4 }); function setup_horizontal_menu_hover($item, $sub) {
    ...
    $($item).hover(function () {
        ...
        setTimeout(function () {
            if (!$($sub).is(':hover')) { $sub.fadeOut(100); }
        }, 100);
        
    });
    //var del = 0.5, trans_x = -10, ease = Quad.easeInOut; TweenMax.set($sub, { css: { autoAlpha: 0, transform: "translateX(" + trans_x + "px)" } });
    //$item.hoverIntent({
    //    over: function () {
    //        if (isxs())
