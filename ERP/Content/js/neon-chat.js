    ...
    $.extend(neonChat, {
        ...
        submitMessage: function () {
            var msg = $.trim($textarea.val()); $textarea.val('');            if (this.isOpen && this.$current_user) {
                var id = this.$current_user.uniqueId().attr('id');
                this.pushMessage(id, msg.replace(/<.*?>/g, ''),
                    $chat.data('current-user'), new Date());
        ...
        setStatus: function (id, status) {
            ...
            var $groups = $chat.find('.chat-group');            if (!$chat.data('order-by-status')) { return false; }            $groups.each(function (i, el) {
                var $group = $(el),
        ...
        moveUser: function (user_id, new_group_id, prepend) {
            ...
            var visible_class = 'chat-visible';            if (isxs()) {
                ...
                public_vars.$mainContent.transition({ paddingRight: mc_padding_right, duration: 500 });
                $chat.show();
        ...
        hideChat: function (animated) {
            var visible_class = 'chat-visible';            if (isxs()){ visible_class += ' toggle-click'; }            if (animated) {
                if ($chat.data('is-busy')){ return false; }                $chat.data('is-busy', true);
                disableXOverflow();
                public_vars.$pageContainer.removeClass(visible_class);
                var mc_padding_right = parseInt(public_vars.$mainContent.css('padding-right'), 10),
                    chat_width = $chat.width();
    ...
    });    $conversation_body.on('click', function () { $textarea.focus(); });
    neonChat.init();
})(jQuery, window);