            ...
            compile: function (element) {
                ...
                return function (scope, element, attrs) {
                    ...
                    if (!element.hasClass('ng-table')) {
                        ...
                        var responsiveTable = element.parent('.table-responsive');
                        responsiveTable.addClass('table-responsive-drc');
                        return responsiveTable.after(paginationTemplate);
                    }
                };
            }
        }
...
//$templateCache.put('ng-table/filters/select.html', '<select ng-repeat="data in column.data" ng-options="data.id as data.title for data in column.data" ng-model="params.filter()[name]" ng-show="filter==\'select\'" class="filter filter-select form-control" name="{{column.filterName}}"><option value="0" selected="selected">All</option></select>');